...
- **`pgoutput`, not `decoderbufs` or `wal2json`.** Ships with stock Postgres 10+, so `postgres:18` needs no extra plugin — and it's the only decoding plugin available on RDS, Aurora and Cloud SQL, so the same job runs unchanged there.
...
**Failover, separately from a restart.** Everything above assumes the *same* Postgres instance comes back — its data directory, and the slot on it, survive intact. A real failover (a standby promoted to primary after the original primary dies) is a different, more dangerous case: before Postgres 17, a promoted node has no way to inherit the primary's slot, so a freshly created one starts from "now," silently skipping any committed-but-unconfirmed changes in between — not a delay, data loss with no error raised. Postgres 17 fixes this with synchronized failover slots (`CREATE SUBSCRIPTION ... WITH (failover = true)` plus `synchronized_standby_slots` on the primary); Postgres 18, which this project runs, carries the same fix forward. This project has no standby at all, so the fix is present but the scenario itself is untested here — see "Not here / next steps."
...
A replayed change after a restore carries the same key and the same `_version` (its LSN), so ClickHouse collapses the duplicate. Proven under an actual TaskManager kill and a Postgres restart in [docs/failure-demo.md](docs/failure-demo.md) — 40 and 80 replayed rows respectively, `resource_inventory_current` correct both times.
...
- **Multiple tables sharing one slot.** Prototyped on a separate branch ([`stretch/multi-table`](../../tree/stretch/multi-table)): two more tables, one shared slot, a schema-driven generic parser and sink instead of hand-written Java per table. Not merged into this pipeline; see [docs/multi-table-stretch.md](docs/multi-table-stretch.md) for what it proves and what it doesn't.
- **Postgres HA / failover.** No standby, no Patroni, no promotion — this project cannot lose data to a failover because there's nothing to fail over to. See "Failover, separately from a restart" above; the fix (PG17+ synchronized failover slots) is present in the Postgres version this runs, but the scenario itself has never been exercised here.
...
| Jackson (`jackson-databind`) | 2.22.3 |
| JUnit Jupiter (test only) | 6.1.3 |
| PostgreSQL | **18.6** |
| ClickHouse server | 26.8.9 |
| CI: `actions/checkout`, `actions/setup-java` | v5 |
...
Note: PostgreSQL was bumped from 16 to 18 (tag [`pre-pg18-deps-upgrade`](../../tree/pre-pg18-deps-upgrade) marks the state before) specifically to carry the PG17+ fix for failover-slot data loss — see "Failover, separately from a restart" above. Jackson and JUnit were bumped for currency; neither needed a code change against this project's usage. Flink and ClickHouse were deliberately left as pinned above.

## Stack

- PostgreSQL 18 (`wal_level=logical`, `REPLICA IDENTITY FULL`)
...
docs/                failure demo results, heartbeat experiment, multi-table stretch
